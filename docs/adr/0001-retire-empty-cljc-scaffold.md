...
Remove `src/app.cljc`, its namespace-load smoke test, and the JVM/Clojure test
...
Application, rendering, input, physics, and game-engine orchestration remain
Kami provider responsibilities. A future portable domain layer must be added
as a separately reviewed `.kotoba` contract with explicit types, effects,
resource bounds, and reference/restricted-JavaScript/Wasm evidence.
